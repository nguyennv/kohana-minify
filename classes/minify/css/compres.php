...
/**
 * Minify_CSS_Compres - A (simple) css minifier with benefits
 * 
 * --
 * 
...
class Minify_CSS_Compres
{
	...
	(
		...
		"opacity"					 => array(array("Minify_CSS_Compres", "_opacity")),
		"text-shadow"				 => array("-moz-text-shadow", "-webkit-text-shadow", "-khtml-text-shadow"),
		"white-space"				 => array(array("Minify_CSS_Compres", "_white_space_pre_wrap"))
	);
	/**
	 * Minifies the Css.
	 * 
	 ...
	 * @return string
	 */
	public static function compres($css, $config = array())
	{
		...
		$remove_empty_blocks		 = $config["remove-empty-blocks"]; 
		$remove_empty_rulesets	 = $config["remove-empty-rulesets"];
		$remove_last_semicolon	 = $config["remove-last-semicolons"];
		$convert_css3_properties	 = $config["convert-css3-properties"];
		$compress_unit_values		= $config["compress-unit-values"];
		$convert_color_values		= $config["convert-color-values"];
		$compress_color_values	 = $config["compress-color-values"];
		$emulate_css3_variables	 = $config["emulate-css3-variables"];
		$remove_tokens				= array(self::T_COMMENT);
		// Remove tokens
		if (!$emulate_css3_variables)
		{
			$remove_tokens = array_merge($remove_tokens, array(self::T_AT_VARIABLES_START, self::T_VARIABLE_DECLARATION, self::T_AT_VARIABLES_END));
		}
		for($i = 0, $l = count($tokens); $i < $l; $i++)
		{
			if (in_array($tokens[$i][0], $remove_tokens))
			{
				unset($tokens[$i]);
			}
		}
		...
		for($i = 0, $l = count($tokens); $i < $l; $i++)
		{
			if ($tokens[$i][0] == self::T_DECLARATION)
			{
				...
				if ($convert_color_values && preg_match("/rgb\s*\(\s*([0-9%]+)\s*,\s*([0-9%]+)\s*,\s*([0-9%]+)\s*\)/iS", $tokens[$i][2], $m))
				{
				...
				if ($compress_color_values && preg_match("/\#([0-9a-f]{6})/iS", $tokens[$i][2], $m))
				{
					$m[1] = strtolower($m[1]);
		...
		for($i = 0, $l = count($tokens); $i < $l; $i++)
		{
			...
			{
				$r .= $tokens[$i][1] . ":" . $tokens[$i][2] . ($remove_last_semicolon && $tokens[$i+1][0] == self::T_AT_FONT_FACE_END ? "" : ";");
			}
			// T_AT_PAGE_START
			...
			{
				$r .= $tokens[$i][1] . ":" . $tokens[$i][2] . ($remove_last_semicolon && $tokens[$i+1][0] == self::T_AT_PAGE_END ? "" : ";");
			}
			// T_SELECTORS
			...
			{
				if ($convert_css3_properties && isset(self::$transformations[$tokens[$i][1]]))
				{
				...
				$r .= $tokens[$i][1] . ":" . $tokens[$i][2] . ($remove_last_semicolon && $tokens[$i+1][0] == self::T_DECLARATIONS_END ? "" : ";");
			}
	...
	{
		// Settings
		$default_scope		= array("all");					 	// Default scope
		...
		$current_state		= self::T_DOCUMENT;				 	// Current state
		$scope				= $default_scope;				 	// Current scope
		$string_char		= null;							 	// String delimiter char
		$is_filter_ws		= TRUE;							 	// Filter double whitespaces?
		...
		for ($i = 0, $l = strlen($css); $i < $l; $i++)
		{
			...
			if ($is_filter_ws && $c == " " && $c == $p)
			{
				continue;
			}
			$buffer .= $c;
			if (strpos($token_chars, $c) !== FALSE)
			{
				// 
				$current_state = $state[count($state) - 1];
				/*
				 * Start of comment
				 */
				if ($p == "/" && $c == "*" && $current_state != self::T_STRING && $current_state != self::T_COMMENT)
				{
					$save_buffer = substr($buffer, 0, -2); // save the buffer (will get restored with comment ending)
					$buffer	 = $c;
					$is_filter_ws = FALSE;
				 ...
				 * End of comment
				 */
				elseif ($p == "*" && $c == "/" && $current_state == self::T_COMMENT)
				{
				 ...
				 * Start of string
				 */
				elseif (($c == "\"" || $c == "'") && $current_state != self::T_STRING && $current_state != self::T_COMMENT && $current_state != self::T_STRING_URL)
				{
					$string_char = $c;
					$is_filter_ws = FALSE;
				 ...
				 * End of string
				 */
				elseif ($c === $string_char && $current_state == self::T_STRING)
				{
				 ...
				 */
				elseif ($c == "(" && ($current_state != self::T_COMMENT && $current_state != self::T_STRING) && strtolower(substr($css, $i - 3, 3) == "url") 
					&& ($current_state == self::T_DECLARATION || $current_state == self::T_FONT_FACE_DECLARATION || $current_state == self::T_PAGE_DECLARATION || $current_state == self::T_VARIABLE_DECLARATION))
				{
				 ...
				 */
				elseif (($c == ")" || $c == "\n") && ($current_state != self::T_COMMENT && $current_state != self::T_STRING) && $current_state == self::T_STRING_URL)
				{
					if ($p == "\\")
					{
						continue;
				 ...
				 */
				elseif ($c == "@" && $current_state == self::T_DOCUMENT && strtolower(substr($css, $i, 6)) == "@media")
				{
					$i			= $i + 6;
					$buffer	 = "";
				 ...
				 */
				elseif ($c == "{" && $current_state == self::T_AT_MEDIA_START)
				{
					$buffer	 = strtolower(trim($buffer, $default_trim . "{"));
					$scope		= $buffer != "" ? array_filter(array_map("trim", explode(",", $buffer))) : $default_scope;
				 ...
				 */
				elseif ($current_state == self::T_AT_MEDIA && $c == "}")
				{
				 ...
				 */
				elseif ($c == "@" && $current_state == self::T_DOCUMENT && strtolower(substr($css, $i, 10)) == "@font-face")
				{
				 ...
				 */
				elseif ($c == ":" && $current_state == self::T_AT_FONT_FACE)
				{
					$property = trim($buffer, $default_trim . ":{");
					$buffer	 = "";
				 ...
				 */
				elseif (($c == ";" || $c == "}" || $c == "\n") && $current_state == self::T_FONT_FACE_DECLARATION)
				{
					$value		= trim($buffer, $default_trim . ";}");
				 ...
				 */
				elseif ($c == "@" && $current_state == self::T_DOCUMENT && strtolower(substr($css, $i, 5)) == "@page")
				{
				 ...
				 */
				elseif ($c == ":" && $current_state == self::T_AT_PAGE)
				{
					$property = trim($buffer, $default_trim . ":{");
					$buffer	 = "";
				 ...
				 */
				elseif (($c == ";" || $c == "}" || $c == "\n") && $current_state == self::T_PAGE_DECLARATION)
				{
					$value		= trim($buffer, $default_trim . ";}");
				 ...
				 */
				elseif ($c == "@" && $current_state == self::T_DOCUMENT &&	strtolower(substr($css, $i, 10)) == "@variables")
				{
					$i			= $i + 10;
					$buffer	 = "";
				 ...
				 */
				elseif ($c == "{" && $current_state == self::T_AT_VARIABLES_START)
				{
					$buffer	 = strtolower(trim($buffer, $default_trim . "{"));
				 ...
				 */
				elseif ($c == ":" && $current_state == self::T_AT_VARIABLES)
				{
					$property = trim($buffer, $default_trim . ":");
					$buffer	 = "";
				 ...
				 */
				elseif (($c == ";" || $c == "}" || $c == "\n") && $current_state == self::T_VARIABLE_DECLARATION)
				{
					$value		= trim($buffer, $default_trim . ";}");
				 ...
				 */
				elseif ($c == "@" && $current_state == self::T_DOCUMENT)
				{
					$buffer	 = "";
				 ...
				 */
				elseif ($c == ";" && $current_state == self::T_AT_RULE)
				{
					$pos		= strpos($buffer, " ");
					$rule	 = substr($buffer, 0, $pos);
					$value		= trim(substr($buffer, $pos), $default_trim . ";");
				 ...
				 * Selector
				 */
				elseif ($c == "," && ($current_state == self::T_AT_MEDIA || $current_state ==	self::T_DOCUMENT))
				{
					$selectors[]= trim($buffer, $default_trim . ",");
					$buffer	 = "";
				}
				/*
				 * Start of ruleset
				 ...
				 */
				elseif ($c == ":" && $current_state == self::T_DECLARATIONS)
				{
					$property = trim($buffer, $default_trim . ":;");
					$buffer	 = "";
				 ...
				 * Declaration: Value
				 */
				elseif (($c == ";" || $c == "}" || $c == "\n") && $current_state == self::T_DECLARATION)
				{
					$value		= trim($buffer, $default_trim . ";}");
	 ...
	 * @return string
	 */
	private static function _opacity($property, $value)
	{
		$ie_value = (int) ((float) $value * 100);
		$r	= "-moz-opacity:" . $value . ";";					 // Firefox < 3.5
		$r .= "-ms-filter: \"alpha(opacity=" . $ie_value . ")\";"; // Internet Explorer 8
		$r .= "filter: alpha(opacity=" . $ie_value . ");zoom: 1;"; // Internet Explorer 4 - 7
		return $r;
	}
	/**
	 ...
	 * @return string
	 */
	private static function _white_space_pre_wrap($property, $value)
	{
