...
/**
 * Class Kohana_Minify_CSS_YUI_Compressor 
 * @package Minify
 *
 * YUI Compressor
...
class Kohana_Minify_CSS_YUI {
	...
	public function compress($css, $linebreakpos = 0)
	{
		...
		$css = preg_replace_callback("@(^|\\})(([^\\{:])+:)+([^\\{]*\\{)@", array($this, '_remove_spaces_cb'), $css);
		...
		$css = preg_replace_callback("@rgb\\s*\\(\\s*([0-9,\\s]+)\\s*\\)@", array($this, '_shorten_rgb_cb'), $css);
		...
		$css = preg_replace_callback("@([^\"'=\\s])(\\s*)#([0-9a-fA-F])([0-9a-fA-F])([0-9a-fA-F])([0-9a-fA-F])([0-9a-fA-F])([0-9a-fA-F])@", array($this, '_shorten_hex_cb'), $css);
		...
		if ($linebreakpos > 0)
		{
			...
			$sb_length = strlen($css);
			while ($i < $sb_length)
			{
				$c = $sb[$i++];
				if ($c === '}' AND $i - $linestartpos > $linebreakpos)
				{
	...
	protected function _remove_spaces_cb($m)
	{
		return str_replace(':', '___PSEUDOCLASSCOLON___', $m[0]);
	}

	protected function _shorten_rgb_cb($m)
	{
	...
	protected function _shorten_hex_cb($m)
	{
		// Test for AABBCC pattern
		if ((strtolower($m[3]) === strtolower($m[4])) AND
				(strtolower($m[5]) === strtolower($m[6])) AND
